...
function showCart() {
    ...
    cart.forEach((item, index) => {
        ...
        itemElement.innerHTML = `
            ...
            <div>
                <label>
                    <input type="radio" name="delivery-option-${index}" value="Package" checked>
                    Package
                </label>
                <label>
                    <input type="radio" name="delivery-option-${index}" value="Delivery" onclick="promptForAddress(${index})">
                    Delivery
                </label>
            </div>
        `;
...
function promptForAddress(index) {
    const address = prompt("Please enter your delivery address:");
    if (address) {
        cart[index].deliveryAddress = address; // Save the address for this item
    } else {
        // Revert selection to "Package" if no address is provided
        document.querySelector(`input[name="delivery-option-${index}"][value="Package"]`).checked = true;
...
function checkout() {
    ...
    cart.forEach(item => {
        ...
        // Determine if delivery or package
        const deliveryOption = item.deliveryAddress ? 'Delivery' : 'Package';
        orderSummary += `${item.name} (x${item.quantity}) - $${itemTotal} (${deliveryOption}`;
        
        // Add address if delivery
        if (item.deliveryAddress) {
            orderSummary += `, Address: ${item.deliveryAddress}`;
        }
        orderSummary += ')\n';
    });
